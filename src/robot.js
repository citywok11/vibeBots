...
export function createRobot(startPos = { x: 0, z: 0 }, options = {}) {
  ...
  function bounceOffWalls(arenaSize, wallHeight = Infinity, wallThickness = 0) {
    ...
    // outer face is at half + halfT. Entities inside the arena bounce off the
    // inner face; entities outside bounce off the outer face.
    const innerFace = half - halfT;
    const outerFace = half + halfT;

    // East / West (X axis)
    if (group.position.x >= 0) {
      // Near the east wall — entity to the right of centre
      if (group.position.x <= half) {
        // Inside the arena: bounce off inner face
        ...
        // Outside the arena: bounce off outer face
      ...
      // Near the west wall — entity to the left of centre
      if (group.position.x >= -half) {
        // Inside the arena: bounce off inner face
